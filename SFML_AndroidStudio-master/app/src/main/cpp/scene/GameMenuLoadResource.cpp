...
bool GameMenu::loadResources()
{
    ...
    if (!m_texPad2.loadFromFile(is::GUI_DIR + "option_pad.png"))             return false;
    ...
    if (!m_texGamePad.loadFromFile(is::GUI_DIR + "game_pad.png"))            return false;

    if (!m_fontTitle.loadFromFile(is::FONT_DIR + "space_ranger_3d_mp_pv.otf"))  return false;
    m_cancelBt.loadResources(m_texToolsPad);

    // load configuration file
    if (m_gameSysExt.fileExist(is::CONFIG_FILE)) m_gameSysExt.loadConfig(is::CONFIG_FILE);

    // game title
    is::createWText(m_fontTitle, m_txtGameTitle, L"is::Engine v" + is::GameVersion::getGameVersion() + L"\n\t  Demo",
    ...
    is::createText(m_fontSystem, m_txtStartGame, lang::pad_new_game[m_gameSysExt.m_gameLanguage],
                   is::getSFMLObjX(m_sprPad1), is::getSFMLObjY(m_sprPad1) - TXT_Y_ON_BT, sf::Color::Blue, true, _PAD_TXT_SIZE);
    is::createText(m_fontSystem, m_txtContinue, lang::pad_continue_game[m_gameSysExt.m_gameLanguage],
                   is::getSFMLObjX(m_sprPad2), is::getSFMLObjY(m_sprPad2) - TXT_Y_ON_BT, true, _PAD_TXT_SIZE);
    is::createWText(m_fontSystem, m_txtPadConfig, lang::pad_controller[m_gameSysExt.m_gameLanguage],
    ...
    is::createText(m_fontSystem, m_txtOptionValue[0], lang::opt_enable_sound[m_gameSysExt.m_gameLanguage],
                   _X_OPT, _Y_OPT, sf::Color(sf::Color(0, 0, 0)), _SIZE);
    is::createText(m_fontSystem, m_txtOptionValue[1], lang::opt_enable_music[m_gameSysExt.m_gameLanguage],
                   _X_OPT, _Y_OPT + _OPT_SPACE, sf::Color(sf::Color(0, 0, 0)), _SIZE);
    is::createText(m_fontSystem, m_txtOptionValue[2], lang::opt_enable_vibrate[m_gameSysExt.m_gameLanguage],
                   _X_OPT, _Y_OPT + _OPT_SPACE * 2.f, sf::Color(sf::Color(0, 0, 0)), _SIZE);
    is::createText(m_fontSystem, m_txtOptionValue[3], lang::opt_game_lang[m_gameSysExt.m_gameLanguage],
    ...
    m_gameSysExt.loadPadConfig(is::GAME_PAD_FILE);
    float _xPos(150.f);
    ...
    is::createWText(m_fontSystem, m_txtPermuteAB, lang::msg_permute_AB[m_gameSysExt.m_gameLanguage] + getABPlace(), 0.f, 0.f, sf::Color(255, 255, 255));
    ...
    is::createWText(m_fontSystem, m_txtMovePad, lang::msg_how_move_control[m_gameSysExt.m_gameLanguage], 0.f, 0.f, sf::Color(255, 255, 255, 80));
    ...
    is::setSFMLObjColor(m_txtSetAlpha, sf::Color::White);
    ...
    is::setSFMLTextOutlineColor(m_recSelectPad, 1.f, sf::Color::Red);
